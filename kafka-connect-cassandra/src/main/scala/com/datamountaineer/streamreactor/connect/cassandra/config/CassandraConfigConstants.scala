...
object CassandraConfigConstants {
  ...
  val DC_AWARE_LOCAL_DC = s"$CONNECTOR_PREFIX.dc.aware.local.dc"
  val DC_AWARE_LOCAL_DC_DOC = "The name of DC as known by Cassandra to be considered local"

  val DC_AWARE_USED_HOSTS_PER_REMOTE_DC = s"$CONNECTOR_PREFIX.dc.aware.used.hosts.per.remote.dc"
  ...
  val DC_AWARE_ALLOW_REMOTE_DCS_FOR_LOCAL_CONSISTENCY_LEVEL = s"$CONNECTOR_PREFIX.dc.aware.allow.remote.dcs.for.local.consistency.level"
